...
scratch repository in the corporate org — `sox-collector-pilot` or similar — and
in it:

1. **Add a trivial workflow** that runs `echo hello` on the runner label from
   Step 2, triggered on tag push.
   *Does it run?* → Actions are enabled and the runner label is valid.

2. **Push a tag** `v0.0.1-test`.
   *Does the push succeed?* → tag creation is permitted for this account.

3. **Have the workflow create a Release** and attach any file.
   *Does the Release appear with the file on it?* → `GITHUB_TOKEN` has
   `contents: write`.

If all three pass, everything the release workflow needs is present, and it can
be written as designed. If any fails, the failure names exactly which
conversation to have — with a reproducible example rather than a hypothetical.

Do this **before** the migration, on the scratch repo. Discovering a blocker
during the migration itself is considerably more expensive.
